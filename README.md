...
6. Restores Stream Deck config from `streamdeck-export/` (if present)
7. Applies macOS defaults
8. Sets Chrome as the default browser
9. Sets Dock layout
10. Applies display configuration (if `displayplacer.conf` exists)
11. Applies wallpaper (if `~/Pictures/wallpaper.jpg` exists)
...
- Casks: `1password`, `1password-cli`, `visual-studio-code`, `ghostty`, `hammerspoon`, `raycast`, `herd`, `jump-desktop`, `font-jetbrains-mono-nerd-font`, `google-chrome`, `slack`, `todoist-app`, `codex-app`, `elgato-stream-deck`
